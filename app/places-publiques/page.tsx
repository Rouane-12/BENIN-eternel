        ...
        {PLACES_PUBLIQUES.map((p, i) => (
          <Reveal key={p.name} delay={i * 0.05}>
            <article className="group relative h-[400px] overflow-hidden border border-white/10 hover:border-white/40 transition">
              <img src={p.img} alt={p.name} className="absolute inset-0 w-full h-full object-cover opacity-40 group-hover:scale-105 group-hover:opacity-60 transition-all duration-1000" />
              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/60 to-transparent" />
              <div className="absolute inset-0 p-8 flex flex-col justify-end">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-[10px] tracking-[0.4em] uppercase text-white/50">{p.city}</span>
                  <div className="w-8 h-[1px] bg-white/20" />
                </div>
                <h3 className="font-display text-3xl mb-4 group-hover:text-white transition-colors">{p.name}</h3>
